        ...
        for model in probe_models:
            try:
                ...
                if resp.status_code == 200:
                    print(f"[TierProbe] OpenRouter OK - model: {model}")
                    return True, model
            except Exception as e:
...
def is_rate_limited(child_id: str) -> bool:
    ...
    if len(times) >= _RATE_MAX:
        print(f"[RATE_LIMIT] Rate limit hit for {child_id} - exceeding {_RATE_MAX} requests per {_RATE_WINDOW}s")
        return True
    times.append(now)
    return False
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    if is_quota:
        ...
        message = "An unexpected error occurred in the backend. Diagnostic details have been logged internally."
...
def _sanitize_input(val: str, max_len: int = 40) -> str:
    """Strip potential prompt injection characters and cap length."""
    import re
    if not val:
        return ""
    # Allow alphanumeric characters, spaces, and Arabic/Urdu characters (\u0600-\u06FF)
    cleaned = re.sub(r"[^\w\s\u0600-\u06FF]", "", val)
    return cleaned[:max_len].strip()

def _sanitize_id(val: str, max_len: int = 32) -> str:
    """Sanitize IDs to prevent session collisions/directory path traversal."""
    import re
    if not val:
        return ""
...
class FixedRuleEngine:
    ...
    def get_adaptation(req: AdaptationRequest) -> dict:
        ...
        return {
            "mode": "baseline",
            "reasoning": f"SOVEREIGN BASELINE: Applied fixed rules for failures={req.consecutive_failures}, success={req.success_rate:.2f}",
            "actions": actions
        }
...
async def startup_tier_probe():
    ...
    active = ("gemini" if _tier_health["gemini"] else
              ...
              "heuristic")
    print(f"[Startup] OK Active AI tier on boot: {active.upper()}")
...
async def evaluate_session(data: AdaptationRequest):
    ...
    """
    data.child_id = _sanitize_id(data.child_id)
    data.category = _sanitize_input(data.category, 50)
    ...
    try:
        ...
        res["mode"] = "baseline_fallback"
        res["active_tier"] = "T3:Heuristic"
        res["reasoning"] = "Tier 1 and Tier 2 both failed or timed out. Falling back to Tier 3 Local FixedRuleEngine."
        return res
...
async def generate_quest(data: QuestRequest):
    ...
    """
    data.child_id = _sanitize_id(data.child_id)
    data.child_name = _sanitize_input(data.child_name, 50)
    data.preferred_category = _sanitize_input(data.preferred_category, 50)
    data.difficulty = _sanitize_input(data.difficulty, 20)
    user_id = data.child_id
    
    # Static fallback quest (used on 429 or parse error)
    fallback_quest = {
        "quest_title": "SOVEREIGN ADVENTURE",
...
async def generate_report(data: ReportRequest):
    ...
    data.child_id = _sanitize_id(data.child_id)
    data.child_name = _sanitize_input(data.child_name, 50)
    data.session_summary = _sanitize_input(data.session_summary, 2000)
    data.therapist_insights = _sanitize_input(data.therapist_insights, 500)
    user_id = data.child_id
